 ...
 * FDA Prior Notice PDFダウンロード自動化スクリプト
 * 管理番号入力からPDFダウンロードまで全自動
 */

const SPREADSHEET_ID = '1xJ8PiYwv_T_FVLEXBUi1Fs8MYqAav7CjknzqieLhbkg';
const FBA_SHEET_NAME = 'FBA送料一覧';

/**
 * 認証情報をプロパティに設定（初回のみ実行）
 */
function setCredentials() {
  const props = PropertiesService.getScriptProperties();
  props.setProperty('FDA_ACCOUNT_ID', 'sky69858');
  props.setProperty('FDA_PASSWORD', '[ここにパスワードを入力]');
...
function getCredentials() {
  const props = PropertiesService.getScriptProperties();
  return {
    accountId: props.getProperty('FDA_ACCOUNT_ID'),
    password: props.getProperty('FDA_PASSWORD')
  };
}

/**
 * 管理番号からEntry Numberを取得
 */
function getEntryInfoByManagementNumber(managementNumber) {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName(FBA_SHEET_NAME);

    if (!sheet) {
      return { error: 'FBA送料一覧シートが見つかりません' };
    }

    const data = sheet.getRange('A1:S100').getValues();

    for (let i = 1; i < data.length; i++) {
      if (data[i][1] == managementNumber) {
        return {
          managementNumber: managementNumber,
          entryNumber: data[i][14] || '',
          trackingNumber: data[i][12] || '',
          planNumber: data[i][13] || '',
          status: data[i][11] || '',
          carrier: data[i][7] || '',
          weight: data[i][5] || '',
          rowIndex: i + 1
    ...
    return { error: '管理番号が見つかりません: ' + managementNumber };
  } catch (error) {
    return { error: 'エラーが発生しました: ' + error.toString() };
  }
}

/**
 * PNSIで既存ドキュメントを取得
 * webEntryIdを使用してPDFを取得
 */
function getPDFFromPNSI(entryNumber) {
  Logger.log('DEBUG: getPDFFromPNSI開始 - Entry: ' + entryNumber);

  try {
    // webEntryIdをスプレッドシートから取得（または既知のIDを使用）
    const webEntryId = getWebEntryId(entryNumber);
    Logger.log('DEBUG: webEntryId - ' + webEntryId);

    if (!webEntryId) {
      return { error: 'webEntryIdが見つかりません: ' + entryNumber };
    }

    // 正しいAPIエンドポイントを使用
    const pnsiUrl = 'https://www.access.fda.gov/v1/pnsi/pnsi-facade/api/generatePdfByWebEntryId?webEntryId=' + webEntryId;
    Logger.log('DEBUG: API URL - ' + pnsiUrl);
    ...
    Logger.log('DEBUG: APIリクエスト送信中...');
    const response = UrlFetchApp.fetch(pnsiUrl, options);

    Logger.log('DEBUG: レスポンスコード - ' + response.getResponseCode());

    if (response.getResponseCode() === 200) {
      Logger.log('DEBUG: PDF取得成功');
      const blob = response.getBlob();
      const size = blob.getSize();
      Logger.log('DEBUG: ファイルサイズ - ' + size + 'バイト');

      // Googleドライブに保存
      const fileName = 'FDA_Entry_' + entryNumber.replace(/[^0-9]/g, '') + '.pdf';
      const rootFolder = DriveApp.getRootFolder();
      const file = rootFolder.createFile(blob);
      file.setName(fileName);

      Logger.log('DEBUG: ファイル保存完了 - ' + fileName);
      ...
      Logger.log('DEBUG: PDF取得失敗 - コード: ' + response.getResponseCode());
      Logger.log('DEBUG: レスポンス内容: ' + response.getContentText().substring(0, 500));
      return { 
        error: 'PDF取得失敗: HTTPコード ' + response.getResponseCode()
      };
    }
  } catch (error) {
    Logger.log('DEBUG: エラー - ' + error.toString());
    return { error: 'エラーが発生しました: ' + error.toString() };
  }
}

/**
 * Entry NumberからwebEntryIdを取得
 * FBA送料一覧シートのP列に保存、または既知のIDを使用
 */
function getWebEntryId(entryNumber) {
  // まずスプレッドシートのP列（webEntryId列）を確認
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName(FBA_SHEET_NAME);
    const data = sheet.getRange('A1:P100').getValues();

    for (let i = 1; i < data.length; i++) {
      if (data[i][14] == entryNumber) { // O列 = Entry Number
        if (data[i][15]) { // P列 = webEntryId
          return data[i][15];
        }
      }
    }
  } catch (e) {
    Logger.log('DEBUG: スプレッドシートからwebEntryId取得失敗 - ' + e.toString());
  }

  // スプレッドシートにない場合は既知のIDを使用（テスト用）
  const knownIds = {
    '###-3460064-6': '28921986'
  };

  return knownIds[entryNumber] || null;
}

/**
 * メイン関数：ポップアップで管理番号を入力してPDFまで自動取得
 */
function getFDAEntryInfoWithPrompt() {
  const ui = SpreadsheetApp.getUi();

  // ポップアップで管理番号を入力
  const response = ui.prompt(
    'FDA Prior Notice ダウンロード\n\n管理番号を入力してください:',
  ...
  const managementNumber = response.getResponseText().trim();

  if (!managementNumber) {
    ui.alert('管理番号を入力してください');
    return;
  }

  const number = parseInt(managementNumber);
  if (isNaN(number)) {
    ui.alert('管理番号は数値で入力してください');
    return;
  }

  Logger.clear();
  Logger.log('='.repeat(60));
  Logger.log('FDA Prior Notice自動取得スクリプト');
  Logger.log('='.repeat(60));
  Logger.log('');
  Logger.log('【ステップ1】管理番号で検索中: ' + number);
  Logger.log('');

  const entryInfo = getEntryInfoByManagementNumber(number);

  if (entryInfo.error) {
    Logger.log('❌ エラー: ' + entryInfo.error);
    ui.alert('❌ エラー: ' + entryInfo.error);
    return;
  }

  Logger.log('✓ Entry Number取得完了');
  Logger.log('');
  Logger.log('【ステップ2】PNSIからPDF取得中...');
  Logger.log('');

  const pdfResult = getPDFFromPNSI(entryInfo.entryNumber);

  if (pdfResult.error) {
    Logger.log('❌ PDF取得失敗: ' + pdfResult.error);
    Logger.log('');
    Logger.log('【取得情報】');
    Logger.log('管理番号: ' + entryInfo.managementNumber);
    Logger.log('Entry Number: ' + entryInfo.entryNumber);
    Logger.log('追跡番号: ' + entryInfo.trackingNumber);
    Logger.log('納品プラン番号: ' + entryInfo.planNumber);
    Logger.log('');
    Logger.log('⚠️ 注意: APIが利用不可の場合、以下のURLで手動取得してください');
    Logger.log('URL: ' + pdfResult.fallbackUrl);
    Logger.log('');
    Logger.log('='.repeat(60));

    ui.alert(
      '⚠️ PDF取得失敗\n\n' +
      '管理番号: ' + entryInfo.managementNumber + '\n' +
      'Entry Number: ' + entryInfo.entryNumber + '\n\n' +
      '以下のURLで手動取得してください:\n' +
      'https://www.access.fda.gov/pnsi-app/#/submissions'
    );
    return;
  }
  ...
  Logger.log('追跡番号: ' + entryInfo.trackingNumber);
  Logger.log('納品プラン番号: ' + entryInfo.planNumber);
  Logger.log('ステータス: ' + entryInfo.status);
  Logger.log('業者: ' + entryInfo.carrier);
  Logger.log('重量: ' + entryInfo.weight + 'kg');
  Logger.log('');
  Logger.log('【PDF情報】');
  Logger.log('ファイル名: ' + pdfResult.fileName);
  Logger.log('ファイルサイズ: ' + pdfResult.size + 'バイト');
  Logger.log('保存場所: Googleドライブ（ルートフォルダ）');
  Logger.log('');
  Logger.log('='.repeat(60));
  ...
  ui.alert(
    ...
    '管理番号: ' + entryInfo.managementNumber + '\n' +
    'Entry Number: ' + entryInfo.entryNumber + '\n' +
    'ファイル: ' + pdfResult.fileName + '\n' +
    'サイズ: ' + pdfResult.size + 'バイト\n\n' +
    'Googleドライブ（ルートフォルダ）に保存されました'
  );
}
...
function buildReviewRequestCandidates() {
  ...
  ordersTable.rows.forEach((row) => {
    ...
    rows.push([
      priorDecisions.get(String(orderId).trim()) || '', // 既存判断を引き継ぐ（無ければ空欄）
      orderId,
  ...
  log_('INFO', 'buildReviewRequestCandidates', '', `Built review request candidates: ${rows.length}`, {});
  sendLineMessageIfConfigured_(`Amazon評価依頼 JP: 送信候補 ${rows.length} 件\n${REVIEW_JP.spreadsheetUrl}`);
}
function sendApprovedReviewRequests() {
  ensureBaseSheets_();
  upsertDefaultSettings_();
  // ★4 修正: persistNgDecisionsToExclusions_() は呼ばない。NGは送信対象外(OKのみ送信)であり、恒久除外は不要。
  const settings = getSettings_();
...
function getActiveExclusions_() {
  ...
  const table = readTable_(sheet);
  const activeValue = String(getSettings_().exclude_active_value || 'TRUE').toUpperCase();
  const orderIds = new Set();
  const asins = new Set();
  table.rows.forEach((row) => {
    const active = String(row.active || '').trim();
    if (active && active.toUpperCase() !== activeValue) return;
...
function sendLineMessageIfConfigured_(message) {
  const props = PropertiesService.getScriptProperties();
...
/***** FBA納品CSV STEP1：作業シートへ取込 *****/

const FBA_CSV_CONFIG = {
  WORK_SS_ID: '1uF0FAZEyCYreS7_2VND2Tg5APd_PlJI93OMxbYw-k9U',
  WORK_SHEET_NAME: 'CSV作業',

REF_SS_ID: '1_PqYPsK8GxSuFJ_Fq_FheExl8-xjHoq2JnpKIZ9zTus',
REF_SHEET_GID: 784651307,

  MASTER_SS_ID: '1exGBAEx99-2Qc9d0DLRiZbygvgIo4FY_NoBG3Nkan-A',
  MASTER_SHEET_NAME: 'FBA Stock List'
};

function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('FBA納品CSV')
    .addItem('① 作業シートへ取込', 'importFbaCsvWorkData')
    .addSeparator()
    .addItem('② Amazon CSV作成', 'createAmazonFbaCsv')
    .addToUi();
}

function importFbaCsvWorkData() {
  const ui = SpreadsheetApp.getUi();

  const response = ui.prompt(
    '対象管理番号入力',
    '管理番号を半角スペース区切りで入力してください\n\n例\n13681 14162 14553 14582',
    ui.ButtonSet.OK_CANCEL
  );

  if (response.getSelectedButton() !== ui.Button.OK) return;

  const targetNumbers = response.getResponseText()
    .trim()
    .split(/\s+/)
    .map(v => String(v).trim())
    .filter(Boolean);
  ...
  const workSs = SpreadsheetApp.openById(FBA_CSV_CONFIG.WORK_SS_ID);
  const workSheet = getOrCreateSheet_(workSs, FBA_CSV_CONFIG.WORK_SHEET_NAME);

  const refSs = SpreadsheetApp.openById(FBA_CSV_CONFIG.REF_SS_ID);
  const refSheet = getSheetByGid_(refSs, FBA_CSV_CONFIG.REF_SHEET_GID);

  const masterSs = SpreadsheetApp.openById(FBA_CSV_CONFIG.MASTER_SS_ID);
  const masterSheet = masterSs.getSheetByName(FBA_CSV_CONFIG.MASTER_SHEET_NAME);

  if (!refSheet) throw new Error('参照シートが見つかりません');
  if (!masterSheet) throw new Error('FBA Stock List が見つかりません');

  const refValues = refSheet.getDataRange().getValues();
  const masterValues = masterSheet.getDataRange().getValues();

  const refHeader = refValues[0].map(normalizeHeader_);
  const masterHeader = masterValues[0].map(normalizeHeader_);

  const refCols = {
    kanri: findCol_(refHeader, ['管理番号']),
    allKanri: findCol_(refHeader, ['全管理番号', '全　管理番号']),
    asin: findCol_(refHeader, ['ＡＳＩＮ', 'ASIN']),
    note: findCol_(refHeader, ['注意点']),
    exp: findCol_(refHeader, ['賞味期限']),
    weight: findCol_(refHeader, ['梱包箱重量kg']),
    length: findCol_(refHeader, ['縦cm']),
    width: findCol_(refHeader, ['横cm']),
    height: findCol_(refHeader, ['高さcm']),
    multiSku: findCol_(refHeader, ['複数SKU'])
  };

  const masterCols = {
    sku: findCol_(masterHeader, ['sku', 'SKU', 'seller-sku', 'Merchant SKU']),
    asin: findCol_(masterHeader, ['asin', 'ASIN', 'asin1', 'ＡＳＩＮ'])
  };

  const asinToSkus = {};
  for (let r = 1; r < masterValues.length; r++) {
    const asin = String(masterValues[r][masterCols.asin] || '').trim();
    const sku = String(masterValues[r][masterCols.sku] || '').trim();
    if (!asin || !sku) continue;
    if (!asinToSkus[asin]) asinToSkus[asin] = [];
    if (!asinToSkus[asin].includes(sku)) asinToSkus[asin].push(sku);
  }
...
const output = [[
  ...
  for (let r = 1; r < refValues.length; r++) {
    const row = refValues[r];

    const kanri = String(row[refCols.kanri] || '').trim();
    const allKanri = String(row[refCols.allKanri] || '').trim();

const rowNumbers = allKanri
  .split(/\s+/)
  .map(v => String(v).trim())
  .filter(Boolean);

const matchedNumbers = targetNumbers.filter(n => rowNumbers.includes(n));

if (matchedNumbers.length === 0) continue;

matchedNumbers.forEach(n => foundNumbers.add(n));

    const asin = String(row[refCols.asin] || '').trim();
    const note = String(row[refCols.note] || '').trim();
    const multiSku = String(row[refCols.multiSku] || '').trim();

    if (multiSku) {
      errors.push(`複数SKU：全管理番号 ${allKanri} / 管理番号 ${kanri}`);
      continue;
    }

    if (!asin) {
      errors.push(`ASINなし：全管理番号 ${allKanri} / 管理番号 ${kanri}`);
      continue;
    }

const skus = asinToSkus[asin] || [];

if (skus.length === 0) {
  errors.push(`SKU未取得：全管理番号 ${allKanri} / 管理番号 ${kanri} / ASIN ${asin}`);
  continue;
}

const validSkus = skus.filter(sku =>
  !String(sku || '').trim().toLowerCase().startsWith('amzn.gr.')
);

if (validSkus.length === 0) {
  errors.push(
    `Amazon生成SKUのみのため採用禁止：全管理番号 ${allKanri} / 管理番号 ${kanri} / ASIN ${asin} / SKU ${skus.join(', ')}`
  );
  continue;
}
...
if (validSkus.length > 1) {
  errors.push(
    `ASIN複数SKU一致：全管理番号 ${allKanri} / 管理番号 ${kanri} / ASIN ${asin} / SKU ${validSkus.join(', ')}`
  );
  continue;
}

const prohibitedSkus = skus.filter(sku =>
  String(sku || '').trim().toLowerCase().startsWith('amzn.gr.')
);

if (prohibitedSkus.length > 0) {
  errors.push(
    `Amazon生成SKUのため採用禁止：全管理番号 ${allKanri} / 管理番号 ${kanri} / ASIN ${asin} / SKU ${prohibitedSkus.join(', ')}`
  );
  continue;
};
...
if (prohibitedSkus.length > 0) {
  ...
  continue;
    ...
    if (skus.length > 1) {
      errors.push(`ASIN複数SKU一致：全管理番号 ${allKanri} / 管理番号 ${kanri} / ASIN ${asin} / SKU ${skus.join(', ')}`);
      continue;
    }

    const qty = extractQuantityFromNote_(note);

    if (!qty) {
      errors.push(`数量未取得：全管理番号 ${allKanri} / 管理番号 ${kanri} / 注意点 ${note}`);
      continue;
    }
...
output.push([
  ...
  'seller',
  'seller',
  formatAmazonDate_(row[refCols.exp]),
  qty,
  1,
  row[refCols.length],
  row[refCols.width],
  ...
  const notFound = targetNumbers.filter(n => !foundNumbers.has(n));

  if (notFound.length > 0) {
    errors.push('管理番号未検出：' + notFound.join(', '));
  }

  workSheet.clearContents();
  workSheet.getRange(1, 1, output.length, output[0].length).setValues(output);
  workSheet.setFrozenRows(1);

  if (errors.length > 0) {
    ui.alert(
      '確認が必要です',
      errors.slice(0, 30).join('\n') + (errors.length > 30 ? `\n他 ${errors.length - 30} 件` : ''),
      ui.ButtonSet.OK
    );
  }

  ui.alert(`取込完了：${output.length - 1}件`);
}
...
function findCol_(headers, names) {
  ...
  throw new Error('列が見つかりません: ' + names.join(' / '));
}

function extractQuantityFromNote_(note) {
  const text = String(note || '');
  const m = text.match(/(\d+)\s*個/);
  return m ? Number(m[1]) : '';
}

function formatAmazonDate_(value) {
  if (!value) return '';
  if (Object.prototype.toString.call(value) === '[object Date]') {
    return Utilities.formatDate(value, 'Asia/Tokyo', 'MM/dd/yyyy');
  }
  return String(value).trim();
}

function debugFbaCsvSource() {
  const refSs = SpreadsheetApp.openById(FBA_CSV_CONFIG.REF_SS_ID);
  const refSheet = getSheetByGid_(refSs, FBA_CSV_CONFIG.REF_SHEET_GID);

  if (!refSheet) {
    SpreadsheetApp.getUi().alert('参照シートが見つかりません');
    return;
  }
  ...
  SpreadsheetApp.getUi().alert(
    '参照シート確認\n\n' +
    'シート名: ' + refSheet.getName() + '\n' +
    '最終行: ' + refSheet.getLastRow() + '\n' +
    '最終列: ' + refSheet.getLastColumn() + '\n\n' +
    '1行目:\n' + headers.join(' / ')
  );
}

function debugFbaMasterHeader() {
  const masterSs = SpreadsheetApp.openById(FBA_CSV_CONFIG.MASTER_SS_ID);
  const masterSheet = masterSs.getSheetByName(FBA_CSV_CONFIG.MASTER_SHEET_NAME);

  if (!masterSheet) {
    SpreadsheetApp.getUi().alert('FBA Stock List が見つかりません');
    return;
  }
  ...
  SpreadsheetApp.getUi().alert(
    'FBA Stock List確認\n\n' +
    'シート名: ' + masterSheet.getName() + '\n' +
    '最終行: ' + masterSheet.getLastRow() + '\n' +
    '最終列: ' + masterSheet.getLastColumn() + '\n\n' +
    '1行目:\n' + headers.join(' / ')
  );
}

function debugAllKanriValues() {
  const refSs = SpreadsheetApp.openById(FBA_CSV_CONFIG.REF_SS_ID);
  const refSheet = getSheetByGid_(refSs, FBA_CSV_CONFIG.REF_SHEET_GID);
  ...
  const col = findCol_(headers, ['全管理番号', '全　管理番号']);

  const sample = values
    .slice(1, 30)
    .map((row, i) => `${i + 2}行目: ${row[col]}`)
    .join('\n');
...
function createAmazonFbaCsv() {
  const ui = SpreadsheetApp.getUi();

  const ss = SpreadsheetApp.openById(FBA_CSV_CONFIG.WORK_SS_ID);
  const sheet = ss.getSheetByName(FBA_CSV_CONFIG.WORK_SHEET_NAME);

  if (!sheet) {
    ui.alert('CSV作業シートが見つかりません');
    return;
  }

  const values = sheet.getDataRange().getValues();
  if (values.length <= 1) {
    ui.alert('CSV作業シートにデータがありません');
    return;
  }

  const header = values[0].map(normalizeHeader_);

  const cols = {
    sku: findCol_(header, ['MerchantSKU']),
    qty: findCol_(header, ['Quantity']),
    prep: findCol_(header, ['Prepowner']),
    label: findCol_(header, ['Labelingowner']),
    exp: findCol_(header, ['Expirationdate(MM/DD/YYYY)']),
    units: findCol_(header, ['Unitsperbox']),
    boxes: findCol_(header, ['Numberofboxes']),
    length: findCol_(header, ['Boxlength(cm)']),
    width: findCol_(header, ['Boxwidth(cm)']),
    height: findCol_(header, ['Boxheight(cm)']),
    weight: findCol_(header, ['Boxweight(kg)']),
    kanri: findCol_(header, ['管理番号']),
    allKanri: findCol_(header, ['全管理番号', '全管理番号'])
  };
  ...
  const groupMap = {};
  data.forEach(row => {
    const groupKey = String(row[cols.allKanri] || '').trim();
    if (!groupKey) return;
  ...
  Object.keys(groupMap).forEach(groupKey => {
    ...
    const representative = Math.min(...nums);

    const repRow = rows.find(row => {
      return Number(String(row[cols.kanri] || '').trim()) === representative;
    });

    if (!repRow) {
      errors.push(`代表番号行が見つかりません：${groupKey}`);
      return;
    }

    const box = {
      length: repRow[cols.length],
      width: repRow[cols.width],
      height: repRow[cols.height],
      weight: repRow[cols.weight]
    };

    if (!box.length || !box.width || !box.height || !box.weight) {
      errors.push(`代表番号 ${representative} の箱サイズ/重量が未入力です`);
      return;
    }

    boxInfoMap[groupKey] = box;
  });
...
csvRows.push([
  ...
  row[cols.units],
  row[cols.boxes],
  cmToInch_(box.length),
  cmToInch_(box.width),
  cmToInch_(box.height),
  kgToLb_(box.weight)
]);
  });

const txt = csvRows.map(row =>
  row.map(v => String(v ?? '')).join('\t')
).join('\r\n');

  const firstGroupKey = Object.keys(groupMap)[0] || 'FBA';
  const nums = firstGroupKey
    .split(/\s+/)
    .map(v => Number(v))
    .filter(n => !isNaN(n));

  const representative = nums.length ? Math.min(...nums) : 'FBA';
  const dateText = Utilities.formatDate(new Date(), 'Asia/Tokyo', 'yyyyMMdd');
  const fileName = `FBA_${representative}_${dateText}.txt`;

  const folder = getOrCreateFolderByName_('FBA納品CSV');
